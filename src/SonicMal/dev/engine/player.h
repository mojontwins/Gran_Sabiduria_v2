...
void player_init (void) {
	// Init player data
	prx = (signed int) ((game_somari ? 2 : player_ini_x [level]) << 4); px = prx << FIX_BITS;
...
void player_move (void) {
	...
	if (evil_tile_hit) {
		...
		px = pcx; py = pcy;
		if (pvy > 0) pvy = -PLAYER_V_REBOUND_MULTI; else pvy = PLAYER_V_REBOUND_MULTI;
#endif
...
*/
	...
	*/
	...
	if (phit) {
		...
		psprid = CELL_JUMP_BASE + ((frame_counter >> 4) & 3);
