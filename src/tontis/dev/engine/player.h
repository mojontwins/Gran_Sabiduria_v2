...
void player_init (void) {
	...
	switch (level) {
		case GM_POTIPOTI:
			...
			break;
		case GM_RENDEZVOUS:
			px = ini_px [active_girl];
			py = ini_py [active_girl];
			prx = px >> FIX_BITS;
			pry = py >> FIX_BITS;
			break;
		case GM_NAPIA:
	...
	pcharacter = girl_pcharacter [active_girl];
...
// CUSTOM {
	if (game_rendezvous && wins > 4) { pal_bg (pal_reds); pkilled = 1; }
// } END_OF_CUSTOM

#ifdef PLAYER_DIE_AND_RESPAWN
	player_render ();
	if (game_napia) player_border ();
	if (!game_rendezvous) music_pause (1);
	delay (60);
	player_reset_movement ();
	if (!game_rendezvous) music_pause (0);
...
void player_move (void) {
	...
	// CUSTOM {
		...
		// Wraparound for POTI POTI
		if (px < 0) px += (256<<FIX_BITS);
		else if (px > (256<<FIX_BITS)) px -= (256<<FIX_BITS);
		prx = px >> FIX_BITS;
	}
	// } END_OF_CUSTOM
