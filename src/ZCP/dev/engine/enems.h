...
// } END_OF_CUSTOM
...
#endif		
		{
			...
			if (game_zcp && ep_killed [rdc]) en_t [gpit] = 0x62; else 
			// } END_OF_CUSTOM
...
// We need a custom solution for ZCP + DARK YUN

void enems_drain (void) {
	if (_en_dying | _en_washit) return;
...
void enems_do (void) {
	...
	gpjt = 3; while (gpjt --) {
		...
		if (_en_washit || _en_dying) {
			...
			if (_en_dying) { en_dying [gpit] --; continue; }
		...
		if (rdt) {
			...
			if (_en_ud) {
				...
				cy1 = cy2 = (_en_y + 15) >> 4;
				cm_two_points ();
				if ((at1 | at2) & UPSIDE_DOWN_STOPPER_BITMASK) {
