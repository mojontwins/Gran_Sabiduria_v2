...
void bullets_fire (void) {
	...
	b_x [rda] = prx + boffsx [boffsoff + rdb];
	b_y [rda] = pry + boffsy [boffsoff + rdb];

	b_ac [rda] = 1;
	
	if (game_cheril) bullets_deplete (); // Custom
	...
	gpjt = BULLETS_MAX; while (gpjt --) {
		...
		if (b_ac [bi]) {
			...
			oam_index = oam_spr (
				...
				BULLET_PATTERN, 1, oam_index
			);

			if (b_x [bi] < 8 || b_x [bi] > 240 || b_y [bi] < 8 || b_y [bi] > 176 || (scr_attr [((b_x [bi] + 4) >> 4) + ((b_y [bi] + 7) & 0xf0)] & 8)) {
				bullets_destroy ();
			}
